...
public class WindowBase : WindowBehaviour
{
    ...
    private List<Dropdown> mDropList = new List<Dropdown>();//所有的Drop
    ...
    {
        CoroutineHelper.Instance.StartCoroutine(Update());
    }
    ...
    public void AddDropdownListener(Dropdown dropdown, UnityAction<int> action)
    {
        if (dropdown != null)
        {
            if (!mDropList.Contains(dropdown))
            {
                mDropList.Add(dropdown);
            }
