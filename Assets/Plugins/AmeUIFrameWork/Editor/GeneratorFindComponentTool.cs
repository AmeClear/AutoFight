...
public class GeneratorFindComponentTool : Editor
{
    ...
    [MenuItem("Assets/生成组件查找脚本(Shift+U) #U", false, 0)]
    static void CreateFindComponentScripts()
    {
        ...
        //设置脚本生成路径
        if (!Directory.Exists(GeneratorConfig.Instance.FindComponentGeneratorPath))
        {
            Directory.CreateDirectory(GeneratorConfig.Instance.FindComponentGeneratorPath);
        }
        //解析窗口组件数据
        ...
        Debug.Log("CsConent:\n" + csContnet);
        string cspath = GeneratorConfig.Instance.FindComponentGeneratorPath + "/" + obj.name + "UIComponent.cs";
